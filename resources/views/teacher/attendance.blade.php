...
document.querySelector('#validate').onclick=async()=>{if(!sessionId||!navigator.onLine){setStatus('Connexion requise pour valider',true);return}try{await sync();await api(`/attendance/sessions/${sessionId}/validate`,{method:'POST'});setStatus("Appel valide");document.querySelector('#validate').disabled=true}catch(error){setStatus('Validation impossible',true)}};
