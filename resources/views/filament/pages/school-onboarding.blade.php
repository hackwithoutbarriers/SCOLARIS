    ...
    <div class="space-y-6">
        <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10">
            <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <div class="text-right"><strong>{{ $this->progress }} %</strong><div class="text-xs text-gray-500">progression</div></div>
            </div>
        ...
        <div class="grid gap-4 md:grid-cols-5">
            @foreach ($steps as $index => $step)
                <a href="{{ $step['url'] }}" class="rounded-xl border p-4 transition hover:border-primary-500 {{ auth()->user()->onboarding_step === $index + 1 ? 'border-primary-500 bg-primary-50 dark:bg-primary-950/30' : 'bg-white dark:bg-gray-900' }}">
