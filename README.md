...
### Premier accès administrateur Render

Le déploiement ne lance volontairement pas les seeders de démonstration.
Dans les variables d'environnement Render du Web Service, définir :

```bash
SCOLARIS_OWNER_EMAIL=admin@votre-ecole.tg
SCOLARIS_OWNER_NAME="Votre Nom" # optionnel
...
Les cookies de session Render sont HTTPS-only, HttpOnly et SameSite=Lax.
Filament conserve sa limitation des tentatives de connexion. Après connexion,
un Enseignant est redirigé directement vers `/teacher/attendance` pour éviter
une étape inutile le matin, tout en conservant l'autorisation par affectation
de classe.
