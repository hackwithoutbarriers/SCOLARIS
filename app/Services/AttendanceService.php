...
class AttendanceService {
    ...
    public function create(array $data, User $user): AttendanceSession {
        ...
        $class = \App\Models\ClassRoom::withoutGlobalScopes()->findOrFail($data['class_room_id']);
    ...
    public function sync(AttendanceSession $session, array $records): array {
        return DB::transaction(function () use ($session, $records): array {
            ...
            if ($session->status === 'VALIDATED') {
                throw ValidationException::withMessages(['session' => 'Validated sessions require a controlled correction.']);
            }
            ...
            foreach ($records as $data) {
                ...
                if (!$student) throw ValidationException::withMessages(['student_id'=>'Student is not enrolled in this class.']);
    ...
    public function correct(AttendanceSession $session, array $records, User $user): array
    {
        ...
        return DB::transaction(function () use ($session, $records, $user): array {
            ...
            foreach ($records as $data) {
                ...
                abort_unless($record, 422, 'Student has no attendance record in this session.');
    ...
    public function validateSession(AttendanceSession $session, User $user): AttendanceSession
    {
        ...
        if ($session->status === 'CANCELLED') {
            throw ValidationException::withMessages(['session' => 'Cancelled sessions cannot be validated.']);
        }
