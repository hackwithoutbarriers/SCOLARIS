...
class Invoice extends Model
{
    ...
    {
        return ['total_amount' => 'integer', 'due_date' => 'date'];
    }

    protected static function booted(): void
    {
        static::saving(function (self $invoice): void {
            if ($invoice->school_id && $invoice->academic_year_id && $invoice->due_date && Term::isClosedForDate((int) $invoice->school_id, (int) $invoice->academic_year_id, $invoice->due_date)) {
                throw ValidationException::withMessages(['invoice' => 'La période financière est clôturée.']);
            }
        });
    }
    ...
    {
        if ($this->status === self::CANCELLED || $this->status === self::DRAFT) {
            return $this;
        }
